...
router.post("/", upload.single("arquivo"), async (req, res) => {
    try {
      ...
      return res.status(500).json({ error: 'Erro ao criar processo', message: error.message });
    }
  });
...
router.put('/:id', upload.single('arquivo'), async (req, res) => {
    try {
        ...
        return res.status(500).send({ error: 'Erro ao atualizar processo', message: error.message });
    }
});
